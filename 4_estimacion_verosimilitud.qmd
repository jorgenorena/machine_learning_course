...
$$
\rho(x_1, ..., x_N) = \rho(x_1)...\rho(x_N)\,,
$$
para alguna distribución $\rho(x)$.

## Propiedades de estimadores

Muchas veces desconocemos la forma exacta de $\rho(x)$ pero tenemos un modelo para producirla. Ese modelo puede depender de unos parámetros libres (masas de partículas, varianzas, algún parámetro experimental, ...) $\mathbf{\theta} = (\theta_1, ..., \theta_p)$. Queremos sacar el valor de esos parámetros a partir de los datos. Nuestra aproximación a ese valor usando los datos se llama un **estimador** $\hat{\theta}_N$, que depende del número de datos tomados $N$.
...
Supongamos que la distribución $\rho(x;\theta)$ de una medición depende de un solo parámetro $\theta$. La función de verosimilitud la habíamos definido como la probabilidad de obtener las observaciones $x_1,...,x_N$ dado un valor de $\theta$
$$
L(\theta) = \rho(x_1,...,x_N|\theta) = \prod_{i=1}^N \rho(x_i;\theta)\,,
$$
...
$$
\rho(x_i; \mu, \theta) = \frac{1}{\sigma\sqrt{2\pi}}e^{-(x_i - \mu)^2/2\sigma^2}\,.
$$
...
Supongamos que tenemos un estimador para $\theta$, pero para el problema que queremos resolver nos interesa más una función $g(\theta)$. En general $\hat{g}(\theta) \neq g(\hat{\theta})$ tal que tenemos que calcular el estimador de la función. 

Esto sí se cumple cuando usamos el estimador de máxima verosimilitud, ya que el punto donde $L$ es máximo es el mismo independientemente de si lo escribimos en función de $\theta$ o de $g(\theta)$.

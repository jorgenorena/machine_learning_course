...
Algo que podemos hacer es minimizar las desviaciones de las predicciones del modelo respecto a los datos. Es decir minimizar el $\chi^2$
$$
...
$$
\operatorname{Var}(\hat{\boldsymbol{\theta}}) = M\operatorname{Var}(y)M^T = MCM^T = (\Phi^TC^{-1}\Phi)^{-1}\,.
$$
...
Ahora queremos empezar a sacar conclusiones a partir de los datos. Una primera conclusión es medir el valor de algún parámetro físico. Por ejemplo, la masa del electrón. Pero no sólo queremos un número, también queremos una medida de la incertidumbre de esa medición.
...
$$
P[\theta_- \leq \theta \leq \theta_+] = \int_{x_-}^{x_+} dx\,\rho(x) = C\,.
$$

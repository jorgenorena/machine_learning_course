...
Una capa convolucional consiste en realizar una convolución de la señal de entrada $\boldsymbol{x}$ con un kernel $\boldsymbol{w}$, sumar un sesgo $b_i$ y aplicar una función de activación $a$ para producir la salida $h$. En realidad se producen varios *canales* $h_i$ a partir de la misma señal de entrada $\boldsymbol{x}$, cada uno con un kernel diferente:
$$
h_{ic} = a\left(\sum_{j = -k}^k w_{cj} x_{i+j} + b_{c}\right)\,,
$$
...
$$
h_{ij} = a\left(\sum_{m = -k}^k \sum_{n = -k}^k w_{mn} x_{i+m, j+n} + b\right)\,,
$$
...
$$
h_{cij} = a\left(\sum_{m = -k}^k \sum_{n = -k}^k w_{cmn} x_{i+m, j+n} + b_c\right)\,.
$$
...
$$
h_{cij} = a\left(\sum_{m = -k}^k \sum_{n = -k}^k \sum_{c' = 1}^{C'} w_{cc',mn} x_{c', i+m, j+n} + b_c\right)\,.
$$

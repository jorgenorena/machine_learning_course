...
Supongamos que los datos subyacentes tienen una distribución de probabilidad $\rho(y)$ y nosotros la aproximamos con una verosimilitud $L(y|\theta)$. Una manera de cuantificar la diferencia entre las dos distribuciones es medir la entropía. Esto se hace mediante la **divergencia de Kullback Liebler**
$$
\begin{multline}
...
Ahora bien, supongamos que estimamos los parámetros maximizando la verosimilitud. Sea $\hat{L}$ el máximo valor la verosimilitud. Akaike demostró que $\ln \hat{L}$ es un estimador sesgado de $\langle \ln L\rangle$. Tal que un estimador no sesgado de la divergencia de Kullback Liebler (a parte constantes) es
$$
AIC = 2p -2\log\hat{L}\,.
$$

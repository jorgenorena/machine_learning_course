...
# Esta realización fue elegida porque muestra una fluctuación pedagógicamente útil:
# AIC tiene un mínimo espurio cerca de 10 predictores, casi degenerado con el modelo verdadero.
np.random.seed(0)

# Parámetros
n_muestras = 100
n_total_variables = 20
variables_reales = [0, 1, 2, 3]  # Solo las cuatro primeras variables influyen en la respuesta
...
En la @fig-mse graficamos el error cuadrático medio para una regresión lineal $\sum_{i=1}^p \beta_i X_i$ a partir de datos ficticios. Los datos reales fueron producidos por sólo cuatro predictores. Sin embargo, vemos que al incluir más y más variables el error cuadrático medio sigue decreciendo.
...
Existen varios criterios para evaluar modelos basados en la teoría de probabilidad.

#### Criterio de información de Akaike

Supongamos que los datos son generados por una distribución verdadera $f(y)$. Esta distribución no tiene por qué pertenecer a la familia de modelos que estamos considerando. Un modelo $\mathcal{M}$ con $k$ parámetros libres asigna a cada dato una densidad $p(y|\theta)$. Para un conjunto de datos independiente $D = \{y_i\}_{i=1}^n$, la log-verosimilitud es
$$
\ell_D(\theta) = \log L_D(\theta) = \sum_{i=1}^n \log p(y_i|\theta)\,.
$$

Queremos escoger el modelo que pierda menos información al reemplazar la distribución verdadera $f$ por la distribución aproximada $p_\theta$. Esa pérdida se mide con la **divergencia de Kullback-Leibler**. Usaremos la notación física
$$
\langle F(y)\rangle_f \equiv \int dy\, f(y)F(y)\,.
$$
Entonces
$$
\begin{aligned}
D_{\rm KL}(f||p_\theta)
&= \int dy\, f(y)\log\left(\frac{f(y)}{p(y|\theta)}\right) \\
&= \langle \log f(y)\rangle_f - \langle \log p(y|\theta)\rangle_f \\
&= {\rm const.} - \langle \log p(y|\theta)\rangle_f \,.
\end{aligned}
$$
El primer término sólo depende de la distribución verdadera. Por lo tanto, comparar modelos usando $D_{\rm KL}$ equivale a comparar su log-verosimilitud media sobre datos nuevos.

El problema es que no conocemos $f$. Estimamos los parámetros con los datos disponibles,
$$
\hat{\theta} = \arg\max_\theta \ell_D(\theta),
\qquad
\hat{L}=L_D(\hat{\theta})\,.
$$
Pero $\log\hat{L}$ no es una estimación honesta del desempeño fuera de muestra. Como los mismos datos se usan para ajustar y evaluar el modelo, la log-verosimilitud máxima queda sesgada hacia valores demasiado grandes. El modelo tiene $k$ perillas y las usa para perseguir fluctuaciones estadísticas.

El resultado asintótico de Akaike, para modelos regulares y $n$ grande, es que
$$
\left\langle \ell_D(\hat{\theta}) \right\rangle_D
=
\left\langle \ell_{D'}(\hat{\theta}) \right\rangle_{D,D'} + k + o(1)\,,
$$
donde $D'$ es otro conjunto de datos independiente, generado por la misma distribución verdadera y con el mismo tamaño que $D$. El promedio $\langle\,\cdot\,\rangle_D$ se toma sobre posibles muestras de entrenamiento, mientras que $\langle\,\cdot\,\rangle_{D,D'}$ se toma sobre la muestra de entrenamiento y una muestra nueva independiente. Es decir: la log-verosimilitud máxima calculada sobre los datos de entrenamiento es optimista por aproximadamente $k$.

Por lo tanto, un estimador aproximadamente no sesgado de la log-verosimilitud esperada fuera de muestra es
$$
\ell_{\rm fuera\ de\ muestra}
\simeq
\log\hat{L} - k\,.
$$
Como queremos minimizar la pérdida de información, cambiamos el signo. Por convención se multiplica además por $2$, lo que pone el resultado en la misma escala que la deviance y que los tests de razón de verosimilitudes. Así se obtiene
$$
AIC = -2\log\hat{L} + 2k\,.
$$

Este factor de $2$ no viene de la definición de la divergencia de Kullback-Leibler. Es una convención de escala. Más precisamente, como $\ell_D$ suma las contribuciones de $n$ datos, el $AIC$ estima $2nD_{\rm KL}$ salvo una constante aditiva independiente del modelo y correcciones subdominantes. Como las constantes aditivas y los factores positivos comunes no cambian qué modelo minimiza la pérdida, se escoge el modelo con menor $AIC$.

El valor absoluto del $AIC$ no es importante; lo importante son las diferencias entre modelos. En general una diferencia $\Delta AIC \lesssim 2$ indica modelos difícilmente distinguibles por este criterio. Una diferencia entre $4$ y $7$ es evidencia moderada contra el modelo con mayor $AIC$. Una diferencia mayor que $10$ suele considerarse evidencia fuerte. En general $\exp((AIC_{\min} - AIC)/2)$ es proporcional al soporte relativo del modelo dentro del conjunto comparado.

``` {python}
#| code-fold: true
#| label: fig-aic
#| fig-cap: AIC para datos sintéticos generados a partir de cuatro predictores.
...
for n_vars in range(1, n_total_variables + 1):
    ...
    # Número de parámetros: predictores + término independiente + varianza del ruido
    k = n_vars + 2
...
Si aplicamos este criterio a nuestro modelo de juguete, vemos una sutileza importante: para esta realización de los datos el $AIC$ tiene un mínimo alrededor de $10$ predictores, aunque la complejidad real usada para generar los datos fue $4$. Esto no contradice el criterio. El $AIC$ es una estimación ruidosa de una cantidad esperada. En esta realización la diferencia entre $4$ y el mínimo espurio es pequeña, del orden de $\Delta AIC \simeq 2$, por lo que no deberíamos interpretar la elección de $10$ predictores como una conclusión fuerte. El criterio sirve para comparar evidencia relativa, no para producir un oráculo infalible que sabe contar parámetros verdaderos.
...
Primero asumimos que las mediciones son i.i.d., tal que $L = \prod_i p(x_i)$ y entonces $\ln L = \sum_i \ln p(x_i)$. Para $n$ grande podemos aproximar $\sum_i \ln p(x_i) \approx n \langle\ln p\rangle \equiv n \rho(\boldsymbol{\beta})$.

Usando eso, aproximamos la integral cerca del máximo. Esta aproximación es buena para $n$ grande
$$
\begin{multline}
P(D|M) \approx \int d^p\beta\,e^{\ln L(\boldsymbol{\beta})} \approx \int d^p\beta\,e^{n\ln \rho(\boldsymbol{\beta})} \\ \approx \int d^p\beta\,e^{n\ln \rho(\hat{\boldsymbol{\beta}}) + n\frac{\partial \ln \rho(\hat{\boldsymbol{\beta}})}{\partial \beta^i \partial \beta^j} (\boldsymbol{\beta}^i - \hat{\boldsymbol{\beta}}^i)(\boldsymbol{\beta}^j - \hat{\boldsymbol{\beta}}^j)} = e^{n\ln \rho(\hat{\boldsymbol{\beta}})}\left(\frac{|\partial^2 \ln \rho /\partial\beta^2|^{-1/2}}{n^{p/2}(2\pi)^{p/2}}\right)\,.
\end{multline}
$$
...
#| code-fold: true
#| label: fig-bic
#| fig-cap: BIC para datos sintéticos generados a partir de cuatro predictores.

import numpy as np

# Lista para guardar los valores de BIC
lista_bic = []

# Número de muestras
n = n_muestras

# Calcular BIC para cada modelo con distinto número de predictores
for n_vars in range(1, n_total_variables + 1):
    X_sub = X[:, :n_vars]
    X_con_bias = np.hstack([np.ones((n_muestras, 1)), X_sub])  # Agregar término independiente
...
# Graficar BIC
plt.figure(figsize=(6, 5))
plt.plot(range(1, n_total_variables + 1), lista_bic, marker='o')
plt.axvline(x=4, color='red', linestyle='--', label='Complejidad real del modelo')
plt.xlabel('Número de predictores incluidos')
...
En esta misma realización el $BIC$ sí alcanza su mínimo en $4$ predictores. Aun así se observa una pequeña bajada local al agregar más variables. La diferencia importante es la escala: el mínimo espurio queda mucho más arriba que el mínimo real. Esa es la lección práctica: no hay que mirar sólo la posición del punto más bajo o la existencia de pequeñas ondulaciones, sino diferencias suficientemente grandes en el criterio.

...
Usamos una función Gabor 1D como modelo, donde ajustaremos dos de sus parámetros. Definimos la función
$$
...
$$
La naturaleza oscilatoria del término $\cos(\phi_0 + \phi_1 x + \phi)$ crea un paisaje de pérdida no convexo con múltiples valles (mínimos locales) y puntos de silla.
...
*   $\nabla\mathcal{J}(\theta_{t-1})$ es el gradiente calculado en el paso actual (puede ser batch, mini-batch o stochastic).

A cantidades que se acumulan de la manera como lo hace $v_t$ se les llama **media móvil exponencial**.

Esto es burdamente similar a una bola pesada rodando por una pendiente. Acumula momento, lo que le permite pasar por pequeños baches y acelerar en las bajadas consistentes. Cabe aclarar que a pesar de llamarse momentum, no corresponde exactamente al momentum definido en la física.
